# Record track for meetings/readings
04 Sep 2025: First Contact  
-Project proposal  
25 Sep 2025: First meeting     
Risk assessment submitted   
23 Oct 2025: Meeting + Elin presentation  
-Readings were given  
11 Nov 2025: Hypothesis formation + & SPM tutorial  
20 Nov 2025: Meeting w/ Prof Altmann  
-Additional discussion on research hypotheses  
27 Nov 2025: Meeting w/ discussion of papers
4 Dec 2025: Meeting w/ discussion of papers, SPMs, hypothesis developing

11 Dec 2025: Meeting w/ discussion of paper, and discussion of experiemnt structure (MEG with cues, data acquistion scheme, etc)

18 Dec 2025: Meeting w/ plans for next term, hhypothesis, and group meeting

15 Jan 2026: Meeting via Teams, further discussion on hypothesis development

22 Jan 2026: Group meeting + discussion


# Pipeline (edited 09 Mar) 
1. Data acquistion  
preparing stimulus & take session
2. Preprocessing data  
MEG data preprocessing  
a) Check data fault (ex. header)  
b) Reconstruction and transformation of MEG  
c) Despiking, line noise removal, band-pass filtering, eye-blink removal, baseline correction, and lastly Gaussian averaging  
d) Downsampling stimlus - match with neural data  
e) Clipping data - from stimluls onset to end of session
e) Additional: transform to fif for python usage

